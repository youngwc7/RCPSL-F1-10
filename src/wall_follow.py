...
class WallFollowNode:
    ...
    def get_agg_error(self, scan_msg, min_angle, max_angle):
        angle = min_angle
        D_t = 0.0
    ...
    def velocity_limiter(self, angle):
        ...
        elif 5.0 <= abs_angle < 13.0:
            return self.low_turn_vel
        elif 13.0  <= abs_angle < 20.0:
    ...
    def scan_callback(self, scan_msg):
        ...
        left_error, _ , _, _ = self.get_agg_error(scan_msg, MIN_ANGLE * DEG_TO_RAD, MAX_ANGLE * DEG_TO_RAD)
        # right side's angles become inverted 
        right_error, _, _, _ = self.get_agg_error(scan_msg, -1 * MAX_ANGLE * DEG_TO_RAD, -1 * MIN_ANGLE * DEG_TO_RAD)
